...
_conn.HAS_IPV6 = False   # maskinen mangler IPv6-rute; overpass-api.de svarer bare på IPv6 uten dette

from .paths import CACHE_DIR

USER_AGENT = "fiskoye-pipeline/0.1 (hobbyprosjekt; leifsebastian@gmail.com)"
...
def cached_get(url: str, params: dict | None = None, *, suffix: str = ".bin",
               data: dict | None = None, json_body: dict | None = None, timeout: int = 180, retries: int = 3,
               subdir: str = "http", min_bytes: int = 1, headers: dict | None = None) -> Path:
    """GET (eller POST når data/json_body er satt). Returnerer sti til cachet fil."""
    d = CACHE_DIR / subdir
    d.mkdir(parents=True, exist_ok=True)
    path = d / (_key(url, params, data or json_body) + suffix)
    ...
    for attempt in range(retries):
        try:
            if data is not None:
                r = _session.post(url, data=data, params=params, timeout=timeout, headers=headers)
            elif json_body is not None:
                r = _session.post(url, json=json_body, params=params, timeout=timeout, headers=headers)
            else:
